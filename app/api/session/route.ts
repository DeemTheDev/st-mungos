...
// An optional { mode: "management" } starts the §8 management-focus viva.
...
import type { SessionMode } from "@/lib/ports";
...
async function readCreateRequest(request: Request): Promise<CreateRequest> {
  ...
  return {
    ...
    // Anything unrecognised falls back to a full station — never silently skip
    // the work-up because of a typo'd param.
    mode: mode === "management" ? "management" : "full",
    isForm: isForm,
  };
}
...
export async function POST(request: Request) {
  ...
  if (!caseId) {
    ...
    const picked = pickRandomCase(summaries, {
      // A management viva only exists for clinical cases (§8).
      stationType: req.mode === "management" ? "clinical" : req.stationType,
  ...
  let state;
  try {
    state = await engine.createSession(caseId, Date.now(), req.mode);
  } catch (err) {
